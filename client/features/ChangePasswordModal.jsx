...
import { encryptPasswordWithTs } from '../utils/security';
...
function ChangePasswordModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const { passwordEnc: currentPasswordEnc, ts } = await encryptPasswordWithTs(form.currentPassword);
      const { passwordEnc: newPasswordEnc } = await encryptPasswordWithTs(form.newPassword);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="修改密码" width="440px" closeOnBackdrop={false} closeOnEscape>
