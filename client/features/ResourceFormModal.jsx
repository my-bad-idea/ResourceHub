...
import { Loader, ChevronDown, X, ListPlus } from 'lucide-react';
...
function ResourceFormModal({ isOpen, onClose, resource, onOpenBatch }) {
  ...
  const handleCreateCategory = useCallback(async (rawValue) => {
  ...
  const commitCategoryInput = useCallback(() => {
        ...
        <div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              >
                {(loading || addingCatLoading) && <Loader size={14} style={{ animation: 'spin 1s linear infinite' }} />}
                保存
              </button>
            </div>
          </div>
