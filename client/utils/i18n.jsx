...
let _useAppState = null;
let _useAppDispatch = null;
export function _injectContextHooks(useAppState, useAppDispatch) {
  _useAppState = useAppState;
  _useAppDispatch = useAppDispatch;
}
function useAppStateLazy() { return _useAppState?.(); }
function useAppDispatchLazy() { return _useAppDispatch?.(); }
...
let __rhPatched = false;
export function initI18nPatch() {
  if (__rhPatched) return;
...
function useI18n() {
  const state = useAppStateLazy();
  const dispatch = useAppDispatchLazy();
...
export {
  RH_SUPPORTED_LOCALES,
  RH_DEFAULT_LOCALE,
  RH_LOCALE_NATIVE_LABELS,
  normalizeLocale,
  detectBrowserLocale,
  applyLocale,
  translateText,
