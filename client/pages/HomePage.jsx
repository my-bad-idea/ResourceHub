...
import { useViewportWidth, getCategoryTone } from '../utils/helpers';
import { LayoutGrid, List, Clock, Plus, ChevronDown, Heart, FileText, Menu, ArrowLeft, X, Tags } from 'lucide-react';
import { DropdownSelect } from '../components/DropdownSelect';
import { AppLayout } from '../layout/AppLayout';
import { ResourceCard } from '../features/ResourceCard';
import { ResourceRow } from '../features/ResourceRow';
import { ResourceTimeline } from '../features/ResourceTimeline';
import { ResourceFormModal } from '../features/ResourceFormModal';
import { BatchResourceModal } from '../features/BatchResourceModal';
...
function HomePage({ pageType = 'overview' } = {}) {
  ...
  const refetchedResourcesRef = React.useRef(false);
  ...
  const categoryList = React.useMemo(() => {
    ...
    const categoriesWithCounts = resolvedCategories.map((category) => {
      ...
      const fromMap = key != null ? categoryCountMap[key] : undefined;
      const resourceCount = quickAccessFilter
        ? (fromMap ?? 0)
        : (fromMap ?? category.resourceCount ?? 0);
      return {
        ...category,
        resourceCount,
      };
  ...
  }, [resolvedCategories, categoryCountMap, basePool.length, quickAccessFilter]);
  ...
  const hasCategoryCounts = (resolvedCategories || []).some((c) => (c.resourceCount ?? 0) > 0);
  React.useEffect(() => {
    if (!isResultsMode || (resources || []).length > 0 || !hasCategoryCounts || refetchedResourcesRef.current) return;
    refetchedResourcesRef.current = true;
    request('/api/resources').then((res) => {
      if (res.ok && Array.isArray(res.data?.data)) dispatch({ type: 'SET_RESOURCES', resources: res.data.data });
    }).catch(() => {});
  }, [isResultsMode, resources, hasCategoryCounts, request, dispatch]);
                  ...
                  {pool.length === 0 ? (
                    ...
                    <div data-rh-resource-list-shell style={{
                      ...
                      {pool.map((resource, index) => (
                        <ResourceRow key={resource.id} resource={resource} onEdit={openEdit} isLast={index === pool.length - 1} />
                      ))}
                    </div>
                  ) : (
                    <div
                    ...
                    >
                      <ResourceTimeline resources={pool} onEdit={openEdit} />
                    </div>
                  )}
                </div>
              </div>
      ...
      <div data-rh-home-overview-categories style={{ display: 'grid', gap: '10px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(columns, Math.max(categoryHighlights.length, 1))}, minmax(0, 1fr))`, gap: '12px' }}>
          {categoryHighlights.map((category) => {
            const categoryTone = category.tone || getCategoryTone(category, category.id);
