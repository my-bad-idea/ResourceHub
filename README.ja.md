...
### npm パッケージとしてのインストールと起動（将来予定）

将来、ResourceHub を npm パッケージとして公開する場合、次のようにインストールできる予定です。

```bash
npm install resourcehub
```

その際は、対応する CLI / Node API のエントリーポイントを提供します。具体的な使い方は npm パッケージ正式公開後に本ドキュメントへ追記します。
