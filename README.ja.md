...
- **パスワード**：`Abc12345`
- **表示名**：`admin`
- **メールアドレス**：`zhanglibo610@gmail.com`
- **補足**：環境変数 `SEED_DEMO=1` かつデータベースにユーザーが存在せず、システムが未初期化の場合にのみ、起動時にこの管理者アカウントとデモデータが自動作成されます（デモ環境向け）。
...
- `SEED_DEMO`（任意。`1` を設定すると、空の DB かつ未初期化の場合に既定のデモ管理者とデモデータを自動作成します）
