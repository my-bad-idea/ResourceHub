...
async function request(
  ...
  url: string,
  payload?: unknown,
  tokenOrOptions?: string | { token?: string; locale?: keyof typeof localeCopy }
) {
  const options =
    typeof tokenOrOptions === 'string' || tokenOrOptions === undefined
      ? { token: tokenOrOptions, locale: 'zh-Hans' as const }
      : { locale: 'zh-Hans' as const, ...tokenOrOptions }
...
async function main() {
  try {
    ...
    const localeRegistrations: Record<keyof typeof localeCopy, { username: string; displayName: string; email: string }> = {
    ...
    let alicePassword = ''
    for (const locale of Object.keys(localeRegistrations) as Array<keyof typeof localeCopy>) {
    ...
    for (const locale of ['en', 'zh-Hant', 'ja'] as const) {
